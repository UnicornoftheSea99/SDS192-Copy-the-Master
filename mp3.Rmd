...
```{r}
map_dfr(c("Marion", "Jackie", "Ariel", "Jamie"), most_unisex_yr)

general_context <- tribble(
  ~year_label, ~vpos, ~hjust, ~name, ~text,
  1934, 0.35, "left", "Jessie", "Most\nunisex year",
  1977, 0.35, "left", "Marion", "Marion Jones wins\ngold in Olympics", 
  2006, 0.35, "right", "Jackie", "Jackie Robinson to\nmajor league", 
  1930, 0.35, "left", "Ariel", "The Little Mermaid\nsways Ariel towards girls",
  1936, 0.35, "left", "Jamie", "Jamie Hunter Cartwright\nappears on Bonanza"
)
```
...
ranked_names <- most_unisex_names %>%
  mutate(fct_rmse = factor(rmse),
         name_rank = min_rank(fct_rmse), 
...
ggplot(data, aes(x = year, y = pct_girls)) +
  ...
  geom_text(
  data = marion_context, family = "Century Gothic",
...
```{r word_count, message=FALSE, echo=FALSE, eval=FALSE}
text_stats()
```
...
- `r standard_button("wrangling")`: We met the Wrangling standard because we utilized functions such as group_by() and group_modify(), as well as mutate to optimize the code for readability and performance. 
- `r standard_button("relational")`: We met the Relational standard because we utilized the inner_join function to include potentially missing data and merge various data frames.
- `r standard_button("reshape")`: We met the Reshape standard because we transformed the various data frames with pivot functions for the lists. 
